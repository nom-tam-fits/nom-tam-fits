...
public final class ArrayFuncs {
    ...
    /**
     * Multiplies all elements of a primitive array with an integer scaling factor.
     * 
     * @param  array                    A (multidimensional) Java array of primitive numerical types (i.e. not composed
     *                                      <code>char[]</code> or <code>boolean[]</code>).
     * @param  factor                   The integer scaling factor
     * 
     * @throws IllegalArgumentException if the object argument is not an array, or if it is an array of an usupported
     *                                      data type.
     * 
     * @see                             #scale(Object, double)
     * @see                             #offset(Object, Number)
     * 
     * @since                           1.20
     */
    public static void scale(Object array, long factor) throws IllegalArgumentException {
        if (!array.getClass().isArray()) {
            throw new IllegalArgumentException("Not an array: " + array.getClass().getName());
        }
        ...
        if (array.getClass().getComponentType().isPrimitive()) {
            if (array instanceof byte[]) {
                ...
                throw new IllegalArgumentException("Cannot scale: " + array.getClass().getComponentType());
            }
        } else {
    ...
    /**
     * Multiplies all elements of a primitive array with a floating-point scaling factor. When applying the scaling to
     * integre-type arrays, the floating-point products are rounded before they are recast back to the original integer
     * types. For example, scaling an integer array element of 3 by a facotr of 0.5, i.e. 3 * 0.5 = 1.5, is rounded to
     * 2.
     * 
     * @param  array                    A (multidimensional) Java array of primitive numerical types (i.e. not composed
     *                                      <code>char[]</code> or <code>boolean[]</code>).
     * @param  factor                   The floating-point scaling factor
     * 
     * @throws IllegalArgumentException if the object argument is not an array, or if it is an array of an usupported
     *                                      data type.
     * 
     * @see                             #scale(Object, long)
     * @see                             #offset(Object, Number)
     * 
     * @since                           1.20
     */
    public static void scale(Object array, double factor) throws IllegalArgumentException {
        if (!array.getClass().isArray()) {
            throw new IllegalArgumentException("Not an array: " + array.getClass().getName());
        }
        ...
        if (array.getClass().getComponentType().isPrimitive()) {
            if (array instanceof byte[]) {
                byte[] a = (byte[]) array;
                for (int i = a.length; --i >= 0;) {
                    a[i] = (byte) Math.round(a[i] * factor);
                }
            } else if (array instanceof short[]) {
                short[] a = (short[]) array;
                for (int i = a.length; --i >= 0;) {
                    a[i] = (short) Math.round(a[i] * factor);
                }
            } else if (array instanceof int[]) {
                int[] a = (int[]) array;
                for (int i = a.length; --i >= 0;) {
                    a[i] = (int) Math.round(a[i] * factor);
                }
            } else if (array instanceof long[]) {
                long[] a = (long[]) array;
                for (int i = a.length; --i >= 0;) {
    ...
    /**
     * Adds an offset value to all elements of a primitive array. The offset value is effectively (down)cast to the type
     * of the array elements before being added. For example, adding 2.5 to an integer array will add (int) (2.5), i.e.
     * 2, to all elements.
     * 
     * @param  array                    A (multidimensional) Java array of primitive numerical types (i.e. not composed
     *                                      <code>char[]</code> or <code>boolean[]</code>).
     * @param  value                    The offset value, usually a type that matches the type of array elements.
     *                                      Otherwise it will be downcast to match, which may result in loss of
     *                                      precision.
     * 
     * @throws IllegalArgumentException if the object argument is not an array, or if it is an array of an usupported
     *                                      data type.
     ...
     * @since                           1.20
     */
    public static void offset(Object array, final Number value) throws IllegalArgumentException {
        if (!array.getClass().isArray()) {
            throw new IllegalArgumentException("Not an array: " + array.getClass().getName());
        }
        ...
        if (array.getClass().getComponentType().isPrimitive()) {
            if (array instanceof byte[]) {
                ...
                float[] a = (float[]) array;
                for (int i = a.length; --i >= 0;) {
                    a[i] += value.floatValue();
                }
            } else if (array instanceof double[]) {
                double[] a = (double[]) array;
                for (int i = a.length; --i >= 0;) {
                    a[i] += value.doubleValue();
                }
            } else {
                throw new IllegalArgumentException("Cannot add offset to: " + array.getClass().getComponentType());
            }
        } else {
            Object[] a = (Object[]) array;
            for (int i = a.length; --i >= 0;) {
