...
public class HCompress implements CompressionScheme {
    ...
    private int[] decode(InputBitStream dis) throws IOException {
        ...
        // Read magic number
        if (m1 != HCompress.code_magic[0] || m2 != HCompress.code_magic[1]) {
            throw new IOException("Bad magic number");
        }

        // read size
        this.nx = dis.readBits(32);
        this.ny = dis.readBits(32);
        int nel = this.nx * this.ny;

        // read scale
        this.scale = dis.readBits(32);
    ...
    public int[] decomp(byte[] input) throws IOException {
        ...
        if (this.tmp == null || this.tmp.length < Math.max(this.nx, this.ny)) {
            ...
            flag = new boolean[Math.max(this.nx, this.ny)];
        }
    ...
    private void digitize(int a[]) {
        ...
        int d = (this.scale + 1) / 2 - 1;

        for (int p = 0; p < a.length; p += 1) {
            a[p] = (a[p] > 0 ? a[p] + d : a[p] - d) / this.scale;
        }
    }
    ...
    private void dodecode(InputBitStream dis, int[] a, int[] nbitplanes) throws IOException {
        ...
        qtreeDecode(a, 0, this.ny, nx2, ny2, nbitplanes[0], dis);
        qtreeDecode(a, ny2, this.ny, nx2, this.ny / 2, nbitplanes[1], dis);
        qtreeDecode(a, this.ny * nx2, this.ny, this.nx / 2, ny2, nbitplanes[1], dis);
        qtreeDecode(a, this.ny * nx2 + ny2, this.ny, this.nx / 2, this.ny / 2, nbitplanes[2], dis);
    ...
    void doencode(int a[], OutputBitStream os, byte[] nbitplanes) throws IOException {
        ...
        int ny2 = (this.ny + 1) / 2;
        os.flush();
        qtreeEncode(a, 0, this.ny, nx2, ny2, nbitplanes[0], os);
        qtreeEncode(a, ny2, this.ny, nx2, this.ny / 2, nbitplanes[1], os);
        qtreeEncode(a, this.ny * nx2, this.ny, this.nx / 2, ny2, nbitplanes[1], os);
        qtreeEncode(a, this.ny * nx2 + ny2, this.ny, this.nx / 2, this.ny / 2, nbitplanes[2], os);
        /*
    ...
    private byte[] encode(int a[]) throws IOException {
         ...
         * write magic value
         */
        for (int element : HCompress.code_magic) {
            os.writeBits(element, 8);
        }
        os.writeBits(this.nx, 32);
        os.writeBits(this.ny, 32);
        os.writeBits(this.scale, 32);
    ...
    private void hinv(int[] a, boolean[] flag) {
        ...
        int log2n = (int) (Math.log(nmax) / HCompress.log2 + 0.5);
        ...
        for (int k = log2n - 1; k > 0; k--) {
             ...
             */
            xunshuffle(a, nxtop, nytop, this.ny);
            yunshuffle(a, nxtop, nytop, this.ny, flag);
            for (int i = 0; i < nxtop - 1; i += 2) {
                int pend = this.ny * i + nytop - 1;

                int p00 = 0, p10 = 0;

                for (p00 = this.ny * i, p10 = this.ny * (i + 1); p00 < pend; p00 += 2, p10 += 2) {
         ...
         * unshuffle in each dimension to interleave coefficients
         */
        xunshuffle(a, this.nx, this.ny, this.ny);
        yunshuffle(a, this.nx, this.ny, this.ny, flag);

        for (int i = 0; i < this.nx - 1; i += 2) {

            int pend = this.ny * i + this.ny - 1;
            int p00 = 0, p10 = 0;
            for (p00 = this.ny * i, p10 = p00 + this.ny; p00 < pend; p00 += 2, p10 += 2) {
                int h0 = a[p00];
                int hx = a[p10];
                int hy = a[p00 + 1];
                int hc = a[p10 + 1];
    ...
    private void htrans(int[] a) {
        ...
        for (int k = 0; k < log2n; k++) {
            ...
            for (i = 0; i < nxtop - oddx; i += 2) {
                int s00 = i * this.ny; /* s00 is index of a[i,j] */
             ...
             */
            for (i = 0; i < nxtop; i++) {
                shuffle(a, this.ny * i, nytop, 1);
            }
            for (int j = 0; j < nytop; j++) {
                shuffle(a, j, nxtop, this.ny);
            }
    ...
    private boolean huffEncode(byte[] a, int n, OutputBitStream buffer, int bmax) throws IOException {
        ...
        for (int i = 0; i < n; i += 1) {
            if (a[i] != 0) {
                 ...
                 */
                buffer.writeBits(HCompress.code[a[i]], HCompress.ncode[a[i]]);
                if (buffer.size() > bmax) {
                    return true;
                }
            }
    ...
    private void qtreeDecode(int[] a, int off, int n, int nqx, int nqy, int nbitplanes, InputBitStream dis) throws IOException {
        ...
        log2n = (int) (Math.log(nqmax) / HCompress.log2 + 0.5);
        if (nqmax > 1 << log2n) {
            log2n += 1;
        }
    ...
    void qtreeEncode(int a[], int offset, int n, int nqx, int nqy, int nbitplanes, OutputBitStream os) throws IOException {
        ...
        int log2n = (int) (Math.log(nqmax) / HCompress.log2 + 0.5);
        if (nqmax > 1 << log2n) {
            log2n += 1;
        }
        ...
        bit_loop: for (int bit = nbitplanes - 1; bit >= 0; bit--) {
            ...
            if (huffEncode(scratch, tnx * tny, bo, bmax)) {
                ...
                if (bo.size() == 0) {
                     ...
                     */
                    os.writeBits(HCompress.code[0], HCompress.ncode[0]);
